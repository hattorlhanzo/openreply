...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, error: API_ERRORS.invalidRequestBody },
      { status: 400 }
    );
  }
  ...
  if (!body.recipientId || !text) {
    return NextResponse.json(
      { success: false, error: API_ERRORS.recipientAndMessageRequired },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const message =
      err instanceof MetaApiError ? err.message : API_ERRORS.failedToSendMessage;

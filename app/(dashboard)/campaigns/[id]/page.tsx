...
export default function CampaignDetailPage() {
  ...
  const metrics = [
    { label: "Отправлено", value: campaign.analytics.sent },
    { label: "Переходы", value: campaign.analytics.clicks },
    { label: "CTR", value: `${campaign.analytics.ctr}%` },
    { label: "Ошибки", value: campaign.analytics.failed },
  ];
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,340px)_1fr]">
      ...
      <div className="space-y-6">
        ...
        <Summary title="Когда кто-то комментирует">
          <div className="flex items-center gap-3">
            {postThumb ? (
              ...
              <div className="grid h-14 w-14 place-items-center rounded bg-surface-hover text-[10px] text-muted">
                {campaign.matchAnyPost || campaign.pendingNextReel ? "Любая" : "Пост"}
              </div>
            )}
        ...
        <Summary title="И в комментарии есть">
          ...
          {publicReplies.length > 0 && (
            <div className="space-y-1.5">
              <p className="text-xs text-muted">Публичный ответ под публикацией</p>
        ...
        {campaign.openingDmEnabled && (
          <Summary title="Подписчик получит первое сообщение">
            <FieldBox>{campaign.openingDmMessage || "Первое сообщение"}</FieldBox>
            <FieldBox>{campaign.openingDmButtonLabel || "Кнопка"}</FieldBox>
          </Summary>
        )}

        {campaign.requireFollow && (
          <Summary title="Сначала нужно подписаться">
            <FieldBox>
        ...
        <Summary title="А затем получит сообщение в Direct">
          <FieldBox>{campaign.dmMessage}</FieldBox>
          {hasLink && (
            <FieldBox>{campaign.linkButtonLabel || "Открыть ссылку"}</FieldBox>
          )}
          {hasSecondLink && (
            <FieldBox>
        ...
        {hasLink && (
          <Summary title="Какая ссылка отправляется">
        ...
        {campaign.followUpEnabled && campaign.followUpMessage && (
          <Summary title="Затем напоминание">
